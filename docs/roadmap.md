...
- [x] **Anthropic-compat 路径 metrics 漏统已修**：根因在 `agent_loop.rs::MessageAccumulator` 的 Usage 事件处理是整条替换语义——Anthropic 的两段 usage（message_start 给 input、message_delta 给 output）被第二条 frame 把 input 清零。改成**字段级 max 合并**：单条完整 usage（OpenAI/Qwen/Google）等价于替换；分多条偏分量的 usage（Anthropic/Kimi Code）各字段都保留。3 个回归测试。
- [ ] 跑通三个真实任务：
  - 查待面试日程并按时间排序
  - 新增公司 + 同步到飞书日历
  - 更新面试结果 + 写复盘

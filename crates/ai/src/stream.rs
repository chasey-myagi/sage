...
/// Parses an SSE data chunk (JSON) into zero or more AssistantMessageEvents.
///
/// Returns multiple events when a single chunk contains both a tool call name
/// AND arguments (common with DashScope/Qwen and similar batch-style providers
/// that send the full tool call in one SSE frame).
pub fn parse_sse_chunk_multi(
    data: &str,
) -> Result<Vec<AssistantMessageEvent>, Box<dyn std::error::Error>> {
    let single = parse_sse_chunk(data)?;
    // After emitting ToolCallStart, check if the same chunk also had arguments.
    // Clone the id so we can later move `single` into the return vec.
    let extra_delta = if let Some(AssistantMessageEvent::ToolCallStart { ref id, .. }) = single {
        let id = id.clone();
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(trimmed) {
            json.get("choices")
                ...
                .and_then(|v| v.as_str())
                .filter(|s| !s.is_empty())
                .map(|args| AssistantMessageEvent::ToolCallDelta {
                    id,
    ...
    let mut result: Vec<AssistantMessageEvent> = single.into_iter().collect();
...
mod tests {
    ...
    // ========================================================================
    // parse_sse_chunk_multi — DashScope/Qwen 批式工具调用（name + arguments 同一 chunk）
    // ========================================================================

    #[test]
    fn test_parse_sse_chunk_multi_name_only() {
        // 只有 name，没有 arguments → 单个 ToolCallStart
        let data = r#"{"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_001","type":"function","function":{"name":"bash","arguments":""}}]},"index":0}]}"#;
    ...
    #[test]
    fn test_parse_sse_chunk_multi_name_and_args() {
        // name + arguments 在同一个 chunk（DashScope 风格）→ ToolCallStart + ToolCallDelta
        let data = r#"{"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_002","type":"function","function":{"name":"read","arguments":"{\"path\":\"/tmp\"}"}}]},"index":0}]}"#;
        let events = parse_sse_chunk_multi(data).unwrap();
        assert_eq!(events.len(), 2, "should emit ToolCallStart + ToolCallDelta");
        assert!(
            matches!(&events[0], AssistantMessageEvent::ToolCallStart { name, .. } if name == "read")
        );
        assert!(
            matches!(&events[1], AssistantMessageEvent::ToolCallDelta { arguments_delta, .. } if !arguments_delta.is_empty())
        );
    }

    #[test]
    fn test_parse_sse_chunk_multi_text_delta_unchanged() {
        // 非工具调用 chunk → 单个 TextDelta，与 parse_sse_chunk 行为一致
        let data = r#"{"choices":[{"delta":{"content":"hello"},"index":0}]}"#;
        let events = parse_sse_chunk_multi(data).unwrap();
        assert_eq!(events.len(), 1);
    ...
    #[test]
    fn test_parse_sse_chunk_multi_done_marker() {
        // [DONE] → 空 vec
        let events = parse_sse_chunk_multi("[DONE]").unwrap();
        assert!(events.is_empty());
    }

    #[test]
    fn test_parse_sse_chunk_multi_empty_string() {
        // 空字符串 → 空 vec
        let events = parse_sse_chunk_multi("").unwrap();
        assert!(events.is_empty());
    }

    #[test]
    fn test_parse_sse_chunk_multi_args_delta_only() {
        // 只有 arguments delta（无 name）→ 单个 ToolCallDelta
    ...
    #[test]
    fn test_parse_sse_chunk_multi_usage_chunk() {
        // usage chunk → 单个 Usage 事件
        let data = r#"{"choices":[],"usage":{"prompt_tokens":50,"completion_tokens":20}}"#;
        let events = parse_sse_chunk_multi(data).unwrap();
        assert_eq!(events.len(), 1);
        assert!(matches!(&events[0], AssistantMessageEvent::Usage(_)));
    }

    #[test]
    fn test_parse_sse_chunk_multi_done_stop() {
        // finish_reason: stop → 单个 Done 事件
        let data = r#"{"choices":[{"delta":{},"finish_reason":"stop","index":0}]}"#;
        let events = parse_sse_chunk_multi(data).unwrap();
        assert_eq!(events.len(), 1);
        assert!(
            matches!(&events[0], AssistantMessageEvent::Done { stop_reason } if *stop_reason == StopReason::Stop)
        );
    }

    #[test]
    fn test_parse_sse_chunk_multi_malformed_json() {
        // 非法 JSON → Err
        let result = parse_sse_chunk_multi("not json");
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_sse_chunk_multi_id_propagated_to_delta() {
        // 当 name + args 在同一 chunk 时，ToolCallDelta 的 id 应与 ToolCallStart 相同
        let data = r#"{"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_x","type":"function","function":{"name":"write","arguments":"{\"file\":\"a.txt\"}"}}]},"index":0}]}"#;
        let events = parse_sse_chunk_multi(data).unwrap();
    ...
    // ========================================================================
    // IncrementalJsonParser — 额外边界场景
    // ========================================================================

    #[test]
    fn test_incremental_parser_unicode_chunks() {
        // 多字节 Unicode 字符分片后重组应正确
        let mut parser = IncrementalJsonParser::new();
        parser.push(r#"{"msg":"你好"#);
        parser.push(r#""}"#);
        let val = parser.complete().unwrap();
        assert_eq!(val["msg"], "你好");
    }

    #[test]
    fn test_incremental_parser_array_json() {
        let mut parser = IncrementalJsonParser::new();
        parser.push(r#"[1,2,3]"#);
        let val = parser.complete().unwrap();
        assert!(val.is_array());
    ...
    #[test]
    fn test_incremental_parser_null_value() {
        let mut parser = IncrementalJsonParser::new();
        parser.push("null");
        let val = parser.complete().unwrap();
        assert!(val.is_null());
    ...
    #[test]
    fn test_incremental_parser_deeply_nested() {
        let mut parser = IncrementalJsonParser::new();
        parser.push(r#"{"a":{"b":{"c":{"d":42}}}}"#);
        let val = parser.complete().unwrap();
        assert_eq!(val["a"]["b"]["c"]["d"], 42);
    }

    // ========================================================================
    // parse_sse_chunk — 更多 finish_reason 边界场景
    // ========================================================================
    ...
    #[test]
    fn test_parse_usage_zero_tokens_ignored() {
        // 全零 usage 不应产生 Usage 事件
        let data = r#"{"choices":[],"usage":{"prompt_tokens":0,"completion_tokens":0}}"#;
        let result = parse_sse_chunk(data).unwrap();
        // 实现可能返回 None 或忽略零值 usage
        match result {
            None => {} // 正确：忽略零 usage
            Some(AssistantMessageEvent::Usage(u)) => {
                assert_eq!(u.input, 0);
    ...
    #[test]
    fn test_parse_usage_with_both_cache_and_reasoning() {
        // cache + reasoning 同时存在
        let data = r#"{"choices":[],"usage":{"prompt_tokens":300,"completion_tokens":100,"prompt_tokens_details":{"cached_tokens":100},"completion_tokens_details":{"reasoning_tokens":50}}}"#;
        let event = parse_sse_chunk(data).unwrap().unwrap();
        match event {
            AssistantMessageEvent::Usage(u) => {
                // input = 300 - 100 = 200
    ...
    #[test]
    fn test_parse_error_object_empty_message() {
        // error 对象但 message 字段为空字符串
        let data = r#"{"error":{"message":"","type":"server_error"}}"#;
        let result = parse_sse_chunk(data);
        // Should produce Error event (possibly with empty/unknown message) or Ok(None)
        match result {
            Ok(Some(AssistantMessageEvent::Error(msg))) => {
                // msg may be empty or "unknown error"
                let _ = msg;
            }
            Ok(None) => {} // acceptable
            Err(_) => {}   // acceptable
    ...
    fn test_parse_whitespace_only_data() {
        // 纯空白数据应当被视为空
        let result = parse_sse_chunk("   \t\n  ").unwrap();
        assert!(result.is_none(), "whitespace-only data should return None");
    }

    #[test]
    fn test_parse_sse_chunk_with_extra_fields() {
        // 包含未知额外字段的 chunk 应正常解析
        let data = r#"{"id":"chatcmpl-abc","object":"chat.completion.chunk","created":1700000000,"model":"gpt-4","system_fingerprint":"fp_xyz","choices":[{"delta":{"content":"hi"},"index":0,"finish_reason":null}]}"#;
        let event = parse_sse_chunk(data).unwrap().unwrap();
        assert!(matches!(event, AssistantMessageEvent::TextDelta(s) if s == "hi"));
    }

    #[test]
    fn test_parse_thinking_delta_priority_over_content() {
        // reasoning_content 应优先于 content 字段（pi-mono 的优先级顺序）
        let data = r#"{"choices":[{"delta":{"reasoning_content":"thinking...","content":"text"},"index":0}]}"#;
        let event = parse_sse_chunk(data).unwrap().unwrap();
        assert!(
            matches!(event, AssistantMessageEvent::ThinkingDelta(s) if s == "thinking..."),
            "reasoning_content should take priority over content"
        );
    }
}

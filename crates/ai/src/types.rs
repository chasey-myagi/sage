...
// Cache retention policy is defined in crate::registry::CacheRetention.
// Use that type directly rather than duplicating here.
...
///
/// Aligns with pi-mono's `AssistantMessageEvent` union type (types.ts).
/// Full 12-variant protocol: start → {text,thinking,toolcall}_{start,delta,end} → done|error.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum AssistantMessageEvent {
    /// Stream has started (no content yet). Corresponds to pi-mono `{ type: "start" }`.
    Start,
    /// A text content block has started. Corresponds to pi-mono `{ type: "text_start"; contentIndex }`.
    TextStart {
        content_index: u64,
    },
    TextDelta(String),
    /// A thinking content block has started. Corresponds to pi-mono `{ type: "thinking_start"; contentIndex }`.
    ThinkingStart {
        content_index: u64,
    },
...
mod tests {
    ...
    #[test]
    fn test_cache_retention_registry_variants() {
        use crate::registry::CacheRetention;
        let r = CacheRetention::Extended;
        assert!(matches!(r, CacheRetention::Extended));
        let s = CacheRetention::Standard;
        assert!(matches!(s, CacheRetention::Standard));
    }
}

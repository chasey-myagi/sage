...
///
/// Mirrors `supportsXhigh` in pi-mono's `models.ts`.
///
/// Supported today:
...
///
/// Mirrors pi-mono's `calculateCost()` in `packages/ai/src/models.ts`.
/// Each token category is billed independently at its per-million rate.
/// Returns a [`Cost`] with per-category and total figures.
...
mod tests {
    ...
    // ========================================================================
    // supports_xhigh — mirrors supportsXhigh in pi-mono's models.ts
    // ========================================================================

    #[test]
    fn test_supports_xhigh_opus_4_6_anthropic_messages() {
        // Anthropic Opus 4.6 (registered under github-copilot) should return true.
        // claude-opus-4.6 contains "opus-4.6" which is the Opus 4.6 pattern.
        let model = resolve_model("github-copilot", "claude-opus-4.6")
            .expect("claude-opus-4.6 must be in catalog under github-copilot");
        assert!(supports_xhigh(&model));
    }

    #[test]
    fn test_supports_xhigh_non_opus_anthropic_returns_false() {
        // Non-Opus Anthropic models should return false.
        let model = resolve_model("anthropic", "claude-sonnet-4-20250514")
            .expect("claude-sonnet-4-20250514 must be in catalog");
        assert!(!supports_xhigh(&model));
    }

    #[test]
    fn test_supports_xhigh_gpt_5_4_returns_true() {
        // GPT-5.4 family (github-copilot provider) should return true.
        let model = resolve_model("github-copilot", "gpt-5.4")
            .expect("gpt-5.4 must be in catalog for github-copilot provider");
        assert!(supports_xhigh(&model));
    }

    #[test]
    fn test_supports_xhigh_gpt_5_2_codex_openrouter_via_constructed_model() {
        // Verify that a model with "opus-4.6" in its id always passes supports_xhigh,
        // regardless of the API type (matches pi-mono's openrouter/anthropic-claude-opus-4.6 test).
        let model = Model {
            id: "anthropic/claude-opus-4.6".into(),
            name: "Claude Opus 4.6 via OpenRouter".into(),
            api: api::OPENAI_COMPLETIONS.into(),
            provider: provider::OPENROUTER.into(),
    ...
    #[test]
    fn test_supports_xhigh_gpt_5_3_codex_returns_true() {
        let model = resolve_model("github-copilot", "gpt-5.3-codex")
            .expect("gpt-5.3-codex must be in catalog");
        assert!(supports_xhigh(&model));
    }

    #[test]
    fn test_supports_xhigh_opus_4_6_hyphenated_id() {
        // "opus-4-6" (hyphenated) variant also matches.
        let model = Model {
            id: "claude-opus-4-6-20251001".into(),
            name: "Claude Opus 4.6".into(),
    ...
    fn test_calculate_cost_zero_usage() {
        let config = make_cost_config(15.0, 75.0, 1.5, 18.75);
    ...
    fn test_calculate_cost_input_only() {
        // 1_000_000 input tokens at $15/M → $15
        let config = make_cost_config(15.0, 75.0, 1.5, 18.75);
        let usage = make_usage(1_000_000, 0, 0, 0);
        let cost = calculate_cost(&config, &usage);
        assert!((cost.input - 15.0).abs() < 1e-9);
        assert_eq!(cost.output, 0.0);
        assert!((cost.total - 15.0).abs() < 1e-9);
    }

    #[test]
    fn test_calculate_cost_output_only() {
        // 500_000 output tokens at $75/M → $37.5
        let config = make_cost_config(15.0, 75.0, 1.5, 18.75);
        let usage = make_usage(0, 500_000, 0, 0);
        let cost = calculate_cost(&config, &usage);
        assert!((cost.output - 37.5).abs() < 1e-9);
        assert!((cost.total - 37.5).abs() < 1e-9);
    }

    #[test]
    fn test_calculate_cost_cache_read() {
        // 2_000_000 cache-read tokens at $1.5/M → $3.0
        let config = make_cost_config(15.0, 75.0, 1.5, 18.75);
        let usage = make_usage(0, 0, 2_000_000, 0);
        let cost = calculate_cost(&config, &usage);
    ...
    fn test_calculate_cost_all_categories() {
        // Typical claude-opus-4 request
        // 100 input @ $15/M = $0.0015
        // 200 output @ $75/M = $0.015
        // 50 cache_read @ $1.5/M = $0.000075
        // 10 cache_write @ $18.75/M = $0.0001875
        let config = make_cost_config(15.0, 75.0, 1.5, 18.75);
        let usage = make_usage(100, 200, 50, 10);
        let cost = calculate_cost(&config, &usage);

        let expected_input = 100.0 * 15.0 / 1_000_000.0;
        let expected_output = 200.0 * 75.0 / 1_000_000.0;
        let expected_cache_read = 50.0 * 1.5 / 1_000_000.0;
        let expected_cache_write = 10.0 * 18.75 / 1_000_000.0;
        let expected_total =
        ...
        assert!((cost.cache_read - expected_cache_read).abs() < 1e-12);
        assert!((cost.cache_write - expected_cache_write).abs() < 1e-12);

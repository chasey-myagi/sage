...
mod tests {
    ...
    // ========================================================================
    // normalize_tool_call_ids — base64 / pipe-separated IDs (issue #1022)
    // ========================================================================

    /// Exact ID from issue #1022 that caused "call_id too long" on OpenRouter.
    /// format: "call_xxx|<400+ char base64 with +/=>"
    const FAILING_TOOL_CALL_ID: &str =
    ...
    #[test]
    fn test_normalize_tool_call_ids_real_failing_id_truncated_to_max_len() {
        // The exact ID from issue #1022 — must be truncated to 64 chars.
        let mut messages = vec![
            LlmMessage::Assistant {
                content: "ok".into(),
                tool_calls: vec![LlmToolCall {
        ...
        // Both IDs must be clamped to exactly 64 chars.
        if let LlmMessage::Assistant { tool_calls, .. } = &messages[0] {
            assert_eq!(tool_calls[0].id.len(), 64, "assistant tool call ID must be ≤ 64");
            // Must contain only safe chars.
            assert!(
                tool_calls[0].id.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-'),
                "normalized ID must only contain [a-zA-Z0-9_-]"
            );
        }
        if let LlmMessage::Tool { tool_call_id, .. } = &messages[1] {
            assert_eq!(tool_call_id.len(), 64, "tool result ID must match");
        }
    }

    #[test]
    fn test_normalize_tool_call_ids_base64_chars_replaced() {
        // IDs with base64 chars (+, /, =) must be sanitized.
        let id_with_base64 = "call_abc+def/ghi=jkl";
        let mut messages = vec![
            LlmMessage::Assistant {
                content: "ok".into(),
                tool_calls: vec![LlmToolCall {
                    id: id_with_base64.into(),
                    function: LlmFunctionCall {
                        name: "bash".into(),
                        arguments: "{}".into(),
                    },
            ...
            LlmMessage::Tool {
                tool_call_id: id_with_base64.into(),
                content: "result".into(),
                tool_name: None,
            },
        ];
        normalize_tool_call_ids(&mut messages, 64);
        if let LlmMessage::Assistant { tool_calls, .. } = &messages[0] {
            assert_eq!(tool_calls[0].id, "call_abc_def_ghi_jkl");
        }
        if let LlmMessage::Tool { tool_call_id, .. } = &messages[1] {
            assert_eq!(tool_call_id, "call_abc_def_ghi_jkl");
        }
    }

    #[test]
    fn test_normalize_tool_call_ids_multiple_calls_all_paired() {
        // Two distinct tool calls — both must be normalized and both Tool results
        // must follow the same mapping.
        let id1 = "call_aaa|long_base64==";
        let id2 = "call_bbb|another+base/64";
        let mut messages = vec![
            LlmMessage::Assistant {
                content: String::new(),
                tool_calls: vec![
            ...
            LlmMessage::Tool {
                ...
                tool_name: Some("tool_b".into()),
            },
        ];
        normalize_tool_call_ids(&mut messages, 64);

        let (norm_id1, norm_id2) =
            if let LlmMessage::Assistant { tool_calls, .. } = &messages[0] {
                (tool_calls[0].id.clone(), tool_calls[1].id.clone())
            } else {
                panic!("expected Assistant");
            };

        // Both must be sanitized (no |, +, /, =).
        for id in [&norm_id1, &norm_id2] {
            assert!(
                id.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-'),
                "ID {id:?} has disallowed chars"
            );
        }

        // Tool results must match.
        if let LlmMessage::Tool { tool_call_id, .. } = &messages[1] {
            assert_eq!(tool_call_id, &norm_id1);
        }
        if let LlmMessage::Tool { tool_call_id, .. } = &messages[2] {
            assert_eq!(tool_call_id, &norm_id2);
        }
    }

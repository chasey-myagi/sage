...
pub struct OAuthLoginCallbacks {
    ...
    pub on_prompt: Box<dyn Fn(OAuthPrompt) -> std::pin::Pin<Box<dyn std::future::Future<Output = String> + Send>> + Send + Sync>,
    ...
    pub on_manual_code_input: Option<Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = String> + Send>> + Send + Sync>>,
}
...
pub trait OAuthProviderInterface: Send + Sync {
    ...
    fn modify_models(&self, models: Vec<crate::types::Model>, _credentials: &OAuthCredentials) -> Vec<crate::types::Model> {
        models
    }
}

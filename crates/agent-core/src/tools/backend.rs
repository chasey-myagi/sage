...
///
/// `workspace_root`: used to resolve relative paths for file I/O operations.
/// `policy`: checked against the original (model-provided) path, before workspace_root
/// resolution. This means policy paths should be absolute and independent of
/// workspace_root. The workspace_root does not restrict or expand what the policy allows.

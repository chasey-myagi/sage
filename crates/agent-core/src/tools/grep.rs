...
/// If the total output exceeds MAX_OUTPUT_LINES lines, truncates with a notice.
pub fn format_grep_results(matches: &[GrepMatch], max_line_length: usize) -> String {
    ...
    if lines.len() > MAX_OUTPUT_LINES {
        let truncated = lines[..MAX_OUTPUT_LINES].join("\n");
        format!(
            "{truncated}\n\n[Output truncated: showing first {MAX_OUTPUT_LINES} of {} lines. Use a more specific pattern to narrow results.]",
            lines.len()
        )
    } else {
        result
...
impl super::AgentTool for GrepTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> super::ToolOutput {
        ...
        match self.0.shell(&cmd, 120).await {
            ...
            Ok(output) => {
                // rg not found: exit-code is non-zero, stdout is empty, stderr
                // is non-empty (text varies by shell/OS, so we avoid matching it).
                let rg_available = output.success || !output.stdout.is_empty();
                if !rg_available && !output.stderr.is_empty() {
                    return error_output(

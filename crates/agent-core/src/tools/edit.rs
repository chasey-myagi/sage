...
impl std::fmt::Display for EditError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            EditError::EmptyOldText => write!(f, "old_text cannot be empty"),
            EditError::NotFound => write!(f, "old_text not found in file"),
            EditError::MultipleMatches(n) => {
                write!(f, "old_text found {n} times — must be unique")
            }
        }
    }
}
...
impl super::AgentTool for EditTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> super::ToolOutput {
        ...
        match apply_edit(&content, &old_text, &new_text) {
            ...
            Err(EditError::EmptyOldText) => error_output("old_text cannot be empty"),

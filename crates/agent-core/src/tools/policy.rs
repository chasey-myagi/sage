...
impl ToolPolicy {
    ...
    /// Checks whether `path` is permitted by this policy.
    /// The check is performed on the normalized path (lexical only, no symlink resolution).
    /// Policy paths and workspace_root are independent: policy enforcement happens
    /// on the model-provided path, not the I/O-resolved path.
    ///
...
///
/// NOTE: This function performs lexical path normalization only.
/// It does NOT resolve symbolic links. A symlink pointing outside the
/// allowed paths (e.g., /allowed/link -> /sensitive/) will pass the
/// starts_with check but resolve to a location outside the allowed area
/// at the OS level. Callers that require symlink safety should use
/// std::fs::canonicalize() before passing the path here, or ensure
/// the execution environment prevents symlink creation.

...
use crate::engine::SageError;
use crate::llm::provider_specs::resolve_provider;
...
/// Classify a provider error response into a `SageError::InvalidModel` if
/// the body indicates the model id is invalid. Returns `None` for all other
/// error categories (rate limit, auth, server errors, etc.).
///
/// `SageError::InvalidModel` is the canonical structured representation — its
/// `#[error(...)]` Display template is the single source of the error-message
/// layout; `format_provider_error` delegates here.
pub fn classify_provider_error(
    ...
    let hint = resolve_provider(provider)
        ...
        .unwrap_or_default();
    Some(SageError::InvalidModel {
        provider: provider.to_string(),
        model_id: model_id.to_string(),
        status,
        provider_error: body_excerpt(body),
        hint,
    })
}
...
mod tests {
    ...
    fn classify_provider_error_returns_some_invalid_model_for_matching_body() {
        ...
        assert!(matches!(err, Some(SageError::InvalidModel { .. })));
    }

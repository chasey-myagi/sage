...
use sage_runtime::agent::Agent;
use sage_runtime::agent_loop::run_agent_loop;
use sage_runtime::event::{AgentEvent, AgentEventSink};
use sage_runtime::llm::keys;
use sage_runtime::llm::models;
use sage_runtime::llm::openai_compat::OpenAiCompatProvider;
use sage_runtime::llm::types::{InputType, Model, ModelCost, ProviderCompat, api};
use sage_runtime::tools::ToolRegistry;
...
async fn handle_execute(
    ...
    for name in &tool_names {
        if let Some(tool) = sage_runtime::tools::create_tool(name) {
    ...
    // 4. Create Agent
    let loop_config = sage_runtime::agent::AgentLoopConfig {
        model,
